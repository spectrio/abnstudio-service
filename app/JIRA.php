...
class Jira //implements JIRAContract
{
    ...
    public function searchAllIssuesUnresolved()
    {
        ...
        // search for all statuses
        $search = "?jql=project=NO%20&maxResults=2";
    ...
    public function createIssue($mtid, $acct, $acctName, $username, $playlists=array(), $url, $startDate, $endDate)
    {
        $this->check_login();

        $playlists = str_replace(',',"\n",$playlists);
        //print_r($mtid);print_r($username);print_r($acct);die();

        $datetime = date('Y-m-d\TH:i:s.\0\0\0', strtotime('now')) . '-0400';
        if($startDate) {
          $startDate = date('Y-m-d\TH:i:s.\0\0\0', strtotime($startDate)) . '-0400';
        }
        if($endDate) {
          $endDate = date('Y-m-d\TH:i:s.\0\0\0', strtotime($endDate)) . '-0400';
        }

        $data = array();
        $data['fields']['project']['key'] = 'NO'; //ITSW
        //$data['fields']['summary'] = 'THIS IS A TEST, PLEASE DELETE';
        $data['fields']['summary'] = "Add to Channel(s) - $acctName ($acct)";
        $data['fields']['description'] = "File to add: $url\n
Please add this MP4 to the channels listed for the above account.\n
(This ticket was automatically generated by ABN WeVideo Service.)\n
Start: $startDate\nEnd: $endDate\nGenerated: $datetime";
        $data['fields']['issuetype']['name'] = 'SWO'; //Bug
        //$data['fields']['customfield_10702'][]['value'] = ''; //Orientations
        if($username) {
          $data['fields']['customfield_10505']['name'] = $username; //DMM (LDAP RAM username)
        }
        $data['fields']['customfield_10705'] = "Customer: $acctName\n
Account: $acct\n
Channels: \n$playlists"; //Paragraph

        if($startDate && $endDate) {
          $data['fields']['customfield_10716'] = $startDate;//'2019-03-22T13:11:30.000-0400';//$startDate; //Spot Start Date YYYY-MM-DDThh:mm:ss.sTZD
          $data['fields']['customfield_10717'] = $endDate; //Spot End Date YYYY-MM-DDThh:mm:ss.sTZD
        }
        ...
        //$response = "TESTDATA";
        Log::info("JIRA API Received: " . print_r($response, true));
        return $response;
    }
