...
class ModifiedTemplate extends Model
{
    ...
    {
        return $query
            ->whereRaw('(job_id != "" AND job_id IS NOT NULL) AND ((job_status != "COMPLETED" AND job_status != "FAILED") OR job_status IS NULL)')
            ->select('tid', 'job_id', 'job_status', 'email', 'mtid', 'publish', 'playlists', 'username', 'acct', 'acct_name', 'start_date', 'end_date');
    }
    ...
    {
	Log::info('Updating Status: '.print_r($jobId, true).' - '.print_r($jobStatus, true));
